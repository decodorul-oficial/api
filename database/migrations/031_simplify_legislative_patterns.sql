-- =====================================================
-- MIGRAȚIA 031: SIMPLIFICAREA PATTERN-URILOR PENTRU DIACRITICE
-- =====================================================
-- Simplifică regex-urile pentru a evita problemele cu diacriticele
...
BEGIN
    ...
    -- Hotărâre (simplificat)
    ELSIF v_normalized_text ~ 'hotarare\s+nr\.?\s*(\d+)\s*/\s*(\d{4})' THEN
        v_type := 'hotarare';
        v_number := regexp_replace(v_normalized_text, 'hotarare\s+nr\.?\s*(\d+)\s*/\s*(\d{4})', '\1', 'g');
        v_year := regexp_replace(v_normalized_text, 'hotarare\s+nr\.?\s*(\d+)\s*/\s*(\d{4})', '\2', 'g');
    
    -- Decret (simplificat)
    ELSIF v_normalized_text ~ 'decret\s+nr\.?\s*(\d+)\s*/\s*(\d{4})' THEN
        v_type := 'decret';
        v_number := regexp_replace(v_normalized_text, 'decret\s+nr\.?\s*(\d+)\s*/\s*(\d{4})', '\1', 'g');
        v_year := regexp_replace(v_normalized_text, 'decret\s+nr\.?\s*(\d+)\s*/\s*(\d{4})', '\2', 'g');
...
COMMENT ON FUNCTION public.normalize_legislative_identifier(TEXT) IS 'Normalizează identificatorii legislative în tip, număr și an pentru rezolvare precisă (diacritice normalizate)';
